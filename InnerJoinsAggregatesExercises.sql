...
--3. How many payments where made for each payment type. Display the PaymentTypeDescription and the count
 ...
 SELECT Pt.PaymentTypeDescription, COUNT(P.PaymentID)
 FROM PaymentType Pt
...
SELECT S.FirstName + ' ' + S.LastName AS 'Stundent Name',
		AVG(Mark)
FROM Registration R
...
--6.what is the highest, lowest and average payment amount for each payment type Description? 
 ...
 MAX(P.Amount) AS 'max', MIN(P.Amount) as 'min', AVG(P.Amount) as 'average'
 FROM Payment P
