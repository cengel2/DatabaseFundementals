...
SELECT FirstName, LastName AS 'Staff Name', DATENAME(mm,DateHired)
FROM Staff
...
SELECT DATENAME(mm, Birthdate) AS 'Month Name', COUNT(1) 
FROM Student
GROUP BY DATENAME(mm, Birthdate)

-- 4. Select the Names of all the students born in December.

SELECT FirstName, LastName AS 'Students'
FROM Student
...
/*
    ...
    RTRIM ( character_expression )												SELECT RIGHT(LTRIM('   Danny   '),5)	
    LOWER ( character_expression )												SELECT LEFT(RTRIM('   Danny'   ))
    UPPER ( character_expression )												SELECT LOWER('AWESOME!')
