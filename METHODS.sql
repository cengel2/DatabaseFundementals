...
--4a. Create a stored procedure that lists the courseid, name, and cost of all available courses - called CourseCalendar
...
AS
    -- Body of procedure here
	SELECT PaymentTypeDescription, FirstName + ' ' + LastName AS 'Student Name'
	FROM PaymentType PT
	INNER JOIN Payment P ON P.PaymentTypeID = PT.PaymentTypeID
	RIGHT OUTER JOIN Student S ON S.StudentID = P.StudentID
